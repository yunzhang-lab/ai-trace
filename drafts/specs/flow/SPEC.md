...
`derivation` 是提炼关联对象，用于记录来源材料如何变成候选对象、合并对象或已确认对象。
...
`Flow` 是 derivation 的主入口。Agent 页面只展示与该 Agent 有关的 derivation 摘要，不承载公有内容本体。
...
- `Home`：系统总入口，看公开仓、私有仓、H5、Obsidian 的关系。
- `Wiki`：共享文档、项目、知识、决策、参考资料。
- `Profile`：偏好、知识、教训筛选。
- `Flow`：会话处理、教训提取、提炼关联、同步、冲突、审计。
- `Agents`：Agent 卡片、私有内容类型、入口文件、关联摘要。
- `Skills`：共享动作手册。
