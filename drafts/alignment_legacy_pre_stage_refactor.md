...
> restored_from: pre-refactor working copy
> restored_at: 2026-06-19
> status: legacy-draft
> note: 本文件用于保留 `docs/ALIGNMENT.md` 在阶段职责收口前的旧内容，避免历史讨论丢失。当前不作为权威口径。
...
| `flow/` | `derivation` | 来源到候选、候选到稳定对象的提炼关联 | shared | 是 | Flow 是主入口，Agent 只引用 |
| `flow/` | `handoff` | 交接摘要 | private + shared | 待定 | 当前不作为第一阶段重点 |
...
| `derivation_id` | 唯一提炼关联标识 | 连接 source -> candidate -> approved object | 是 |
...
页面边界只在本文件保留总原则：每个页面必须有明确主语，纯公有内容从对象入口访问，Agent 页面只做 Agent 档案与关联入口。

详细规则见：

- `definitions/agent/README.md`
- `definitions/flow/INFORMATION_FLOW_SPEC.md`
